#!/usr/bin/env python3
"""
Generate a story chapter by chapter using Claude AI with thinking capability.
...
Usage:
    ...
    ./generate_simple.py --seed seeds/chalion.md --num-chapters 15
    ./generate_simple.py --seed seeds/chalion.md --num-chapters 10 --genre fantasy
    ./generate_simple.py --seed seeds/chalion.md --num-chapters 20 --skip-revisions
    ./generate_simple.py --seed seeds/chalion.md --num-chapters 15 --revision-context 500000
"""
...
def extract_chapter_outline(client: anthropic.Anthropic, seed_content: str, chapter_num: int) -> str:
    ...
    try:
        # Define a function to call Claude and process the response
        def call_claude_for_outline():
            # Call Claude to extract the chapter outline
            ...
            # Create a simple response-like object with content
        ...
        response = retry_with_exponential_backoff(call_claude_for_outline)
...
def generate_chapter(
    ...
    temperature: float = 1.0,  # Must be exactly 1.0 when using thinking
    skip_revisions: bool = False,
    revision_context: int = 400000,
    writing_style: str = None,
    genre: str = "fiction"
) -> str:
    """Generate a single chapter using Claude with context from previous chapters.
    ...
    Each step uses Claude's thinking capability for better results.
    """
    ...
    if seed_content:
        chapter_outline = extract_chapter_outline(client, seed_content, chapter_num)
...
"""
    ...
    # Generate initial chapter content with Claude
    messages = [
        {
            "role": "user",
            "content": prompt
    ...
    try:
        ...
        # Define a function to call Claude and process the response
        def call_claude_for_generation():
            response = client.messages.create(
                model="claude-3-7-sonnet-20250219",
                max_tokens=30000,
        ...
        # Call with retry logic
        response = retry_with_exponential_backoff(call_claude_for_generation)
        ...
        # Save Claude's response to the responses directory
        if story_title:
            story_name = story_title.lower().replace(" ", "_")
        else:
            story_name = "unnamed_story"
...
def critique_for_consistency(
    ...
    with the established narrative. Context management ensures we don't exceed Claude's limits.
    """
...
"""
    ...
    try:
        ...
        # Call with retry logic
        response = retry_with_exponential_backoff(call_claude_for_critique)
...
def revise_for_consistency(
    ...
    try:
        ...
        # Call with retry logic
        response = retry_with_exponential_backoff(call_claude_for_revision)
...
def critique_for_style(
    ...
    try:
        ...
        # Define a function to call Claude and process the response
        def call_claude_for_style_critique():
            response = client.messages.create(
                model="claude-3-7-sonnet-20250219",
                max_tokens=30000,
                temperature=1.0,  # Must be exactly 1.0 when using thinking
        ...
        # Call with retry logic
        response = retry_with_exponential_backoff(call_claude_for_style_critique)
...
def revise_for_style(
    ...
    try:
        ...
        # Define a function to call Claude and process the response
        def call_claude_for_style_revision():
            response = client.messages.create(
                model="claude-3-7-sonnet-20250219",
                max_tokens=30000,
                temperature=1.0,  # Must be exactly 1.0 when using thinking
        ...
        # Call with retry logic
        response = retry_with_exponential_backoff(call_claude_for_style_revision)
...
# Removed extract_chapter_count function for simplicity

def extract_writing_style(client: anthropic.Anthropic, seed_path: str, genre: str = "fiction") -> str:
    ...
    try:
        ...
        # Define a function to call Claude and process the response
        def call_claude_for_writing_style():
            response = client.messages.create(
                model="claude-3-7-sonnet-20250219",
                max_tokens=10000,
                temperature=1.0,
        ...
        # Call with retry logic
        response = retry_with_exponential_backoff(call_claude_for_writing_style)
...
def generate_story(
    ...
    # Initialize Claude client - will use thinking capability in all API calls
    client = anthropic.Anthropic()
    ...
    if writing_style is None and seed_path:
        writing_style = extract_writing_style(client, seed_path, genre=genre)
    ...
    for chapter_num in range(1, num_chapters + 1):
        ...
        # Generate chapter content with Claude including previous chapters as context
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--temperature", type=float, default=1.0, help="Temperature for Claude generation (must be 1.0 when using thinking)")
...
def main():
    ...
    # Ensure temperature is exactly 1.0 when using thinking capability
    if args.temperature != 1.0:
        print("WARNING: Setting temperature to 1.0 as required when using Claude's thinking capability")
    
    # Extract writing style guidance
    writing_style = extract_writing_style(client, args.seed, genre=args.genre)
